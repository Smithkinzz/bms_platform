...
// Flux query for Humid alert data
const fluxQuery = `
  from(bucket: "${bucket}")
    |> range(start: -7d)
    |> filter(fn: (r) => r._measurement == "Humid_alert")
...
const AL_Humid = () => {
  ...
  const [formValue, setFormValue] = useState(40);
  const [toValue, setToValue] = useState(60);
  ...
  const handleSwitchChange = (event) => {
    ...
    const data = `Humid_alert switchValue=${newSwitchValue}`;
  ...
  const handleSave = () => {
    setIsEditing(false);
    const data = `
      Humid_alert formValue=${formValue},toValue=${toValue},switchValue=${switchValue},groupValue="${groupValue}",priorityValue="${priorityValue}"
    `;
    axios.post(`${url}api/v2/write`, data, {
      params: { org, bucket },
      headers: {
