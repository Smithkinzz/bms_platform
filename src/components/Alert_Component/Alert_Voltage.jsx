...
// Flux query for voltage alert data
const fluxQuery = `
  from(bucket: "${bucket}")
    |> range(start: -7d)
    |> filter(fn: (r) => r._measurement == "voltage_alert")
...
const AL_Voltage = () => {
  ...
  const handleSwitchChange = (event) => {
    ...
    const data = `voltage_alert switchValue=${newSwitchValue}`;
  ...
  const handleSave = () => {
    setIsEditing(false);
    const data = `
      voltage_alert formValue=${formValue},toValue=${toValue},switchValue=${switchValue},groupValue="${groupValue}",priorityValue="${priorityValue}"
    `;
    axios.post(`${url}api/v2/write`, data, {
      params: { org, bucket },
      headers: {
