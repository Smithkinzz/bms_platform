...
// Flux query for Temp alert data
const fluxQuery = `
  from(bucket: "${bucket}")
    |> range(start: -7d)
    |> filter(fn: (r) => r._measurement == "Temp_alert")
...
const AL_Temp = () => {
  ...
  const [toValue, setToValue] = useState(30);
  ...
  const handleSwitchChange = (event) => {
    ...
    const data = `Temp_alert switchValue=${newSwitchValue}`;
  ...
  const handleSave = () => {
    setIsEditing(false);
    const data = `
      Temp_alert formValue=${formValue},toValue=${toValue},switchValue=${switchValue},groupValue="${groupValue}",priorityValue="${priorityValue}"
    `;
    axios.post(`${url}api/v2/write`, data, {
      params: { org, bucket },
      headers: {
