...
// Flux query for PM10 alert data
const fluxQuery = `
  from(bucket: "${bucket}")
    |> range(start: -7d)
    |> filter(fn: (r) => r._measurement == "PM10_alert")
...
const AL_PM10 = () => {
  ...
  const handleSwitchChange = (event) => {
    ...
    const data = `PM10_alert switchValue=${newSwitchValue}`;
  ...
  const handleSave = () => {
    setIsEditing(false);
    const data = `
      PM10_alert formValue=${formValue},toValue=${toValue},switchValue=${switchValue},groupValue="${groupValue}",priorityValue="${priorityValue}"
    `;
    axios.post(`${url}api/v2/write`, data, {
      params: { org, bucket },
      headers: {
