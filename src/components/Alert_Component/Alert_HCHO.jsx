...
// Flux query for HCHO alert data
const fluxQuery = `
  from(bucket: "${bucket}")
    |> range(start: -7d)
    |> filter(fn: (r) => r._measurement == "HCHO_alert")
...
const AL_HCHO = () => {
  ...
  const [toValue, setToValue] = useState(0.1);
  ...
  const handleSwitchChange = (event) => {
    ...
    const data = `HCHO_alert switchValue=${newSwitchValue}`;
  ...
  const handleSave = () => {
    setIsEditing(false);
    const data = `
      HCHO_alert formValue=${formValue},toValue=${toValue},switchValue=${switchValue},groupValue="${groupValue}",priorityValue="${priorityValue}"
    `;
    axios.post(`${url}api/v2/write`, data, {
      params: { org, bucket },
      headers: {
