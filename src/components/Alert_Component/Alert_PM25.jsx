...
// Flux query for PM2.5 alert data
const fluxQuery = `
  from(bucket: "${bucket}")
    |> range(start: -7d)
    |> filter(fn: (r) => r._measurement == "PM25_alert")
...
const AL_PM25 = () => {
  ...
  const [toValue, setToValue] = useState(50);
  ...
  const handleSwitchChange = (event) => {
    ...
    const data = `PM25_alert switchValue=${newSwitchValue}`;
  ...
  const handleSave = () => {
    setIsEditing(false);
    const data = `
      PM25_alert formValue=${formValue},toValue=${toValue},switchValue=${switchValue},groupValue="${groupValue}",priorityValue="${priorityValue}"
    `;
    axios.post(`${url}api/v2/write`, data, {
      params: { org, bucket },
      headers: {
