...
// Flux query for CO2 alert data
const fluxQuery = `
  from(bucket: "${bucket}")
    |> range(start: -7d)
    |> filter(fn: (r) => r._measurement == "CO2_alert")
...
const AL_CO2 = () => {
  ...
  const [formValue, setFormValue] = useState(400);
  const [toValue, setToValue] = useState(1000);
  const [groupValue, setGroupValue] = useState('Maintenance');
  const [priorityValue, setPriorityValue] = useState('Moderate');
  ...
  const handleSwitchChange = (event) => {
    ...
    const data = `CO2_alert switchValue=${newSwitchValue}`;
  ...
  const handleSave = () => {
    setIsEditing(false);
    const data = `
      CO2_alert formValue=${formValue},toValue=${toValue},switchValue=${switchValue},groupValue="${groupValue}",priorityValue="${priorityValue}"
    `;
    axios.post(`${url}api/v2/write`, data, {
      params: { org, bucket },
      headers: {
